<div align="center">

  ## Hi 👋, I'm HEESUN !

  ### 🚀 Current Status
  **신한투자증권 프로 디지털 아카데미 7기** 교육생입니다.<br>
  백엔드 개발과 인프라 구축에 관심을 가지고 성장하고 있습니다.

  <br>

  ### 🛠 Tech Stack

  **Frontend**<br>
  <img src="https://img.shields.io/badge/React-61DAFB?logo=react&logoColor=black">
  <img src="https://img.shields.io/badge/Next.js-000000?logo=nextdotjs&logoColor=white">
  <img src="https://img.shields.io/badge/Android-3DDC84?logo=android&logoColor=white">

  **Backend**<br>
  <img src="https://img.shields.io/badge/SpringBoot-6DB33F?logo=springboot&logoColor=white">
  <img src="https://img.shields.io/badge/JPA-59666C?logo=java&logoColor=white">

  **Database**<br>
  <img src="https://img.shields.io/badge/MySQL-4479A1?logo=mysql&logoColor=white">
  <img src="https://img.shields.io/badge/Oracle-F80000?logo=oracle&logoColor=white">
  <img src="https://img.shields.io/badge/AWS%20RDS-527FFF?logo=amazonaws&logoColor=white">

  **Infra & DevOps**<br>
  <img src="https://img.shields.io/badge/AWS%20EC2-FF9900?logo=amazonaws&logoColor=white">
  <img src="https://img.shields.io/badge/Docker-2496ED?logo=docker&logoColor=white">
  <img src="https://img.shields.io/badge/Kubernetes-326CE5?logo=kubernetes&logoColor=white">
  <img src="https://img.shields.io/badge/GitHub%20Actions-2088FF?logo=githubactions&logoColor=white">

  **Languages**<br>
  <img src="https://img.shields.io/badge/C-A8B9CC?logo=c&logoColor=white">
  <img src="https://img.shields.io/badge/Java-007396?logo=openjdk&logoColor=white">
  <img src="https://img.shields.io/badge/Python-3776AB?logo=python&logoColor=white">
  <img src="https://img.shields.io/badge/JavaScript-F7DF1E?logo=JavaScript&logoColor=black">

</div>

<br>

## 💻 Experiences

#### 🎓 Education & Activities
- **신한투자증권 프로 디지털 아카데미 7기** (2025.12 - 진행 중)
- **UMC 앱/웹 런칭 동아리**
  - 8기 교내 부리더 (2025.03 - 2025.08)
  - 7기 교내 리더 / SpringBoot 챌린저 (2024.09 - 2025.02)
  - 6기 Android 파트장 (2024.03 - 2024.08)
  - 5기 Web 챌린저 (2023.09 - 2024.02)
- **SW 창의캠프 CJ UNIT 10기** (2024.03 - 2024.07)
- **한국 장학재단 사회리더 대학생 멘토링** (멘티) (2023.04 - 2023.12)
- **EBS 교육 멘토링** (멘토) (2023.03 - 2023.07)

<br>

## 🏆 Awards & Certificates

#### Awards
- **CJ SW 창의캠프 베스트 유닛상** (2024.07)
- **CJ SW 창의캠프 베스트 팀워크상** (2024.07)

#### Certificates
- **정보처리기사** (2025.09)
- **네트워크관리사 2급** (2025.09)

<br>

## 📂 Projects

### 1. ZipCheck
> **전세 사기 예방을 위한 계약 단계별 체크리스트 앱**

- **Role**: Backend Developer
- **Tech**: ![Spring Boot](https://img.shields.io/badge/SpringBoot-6DB33F?logo=springboot&logoColor=white) ![Kubernetes](https://img.shields.io/badge/Kubernetes-326CE5?logo=kubernetes&logoColor=white)
- **GitHub**: [Link](https://github.com/Somsomcore)
- **Key Features**:
- **Risk Assessment**: 주소 기반 유사 매물(아파트/오피스텔/다가구 등) 가격 비교 및 전세 위험도 진단 로직 구현
- **Server Deployment**: 네이버 클라우드 플랫폼(NCP) 및 Kubernetes 기반 배포 환경 구축

<br>

### 2. 티끌모으기 (TikkleSaver)
> **친구와 함께 소비 패턴을 분석하고 관리하는 절약 챌린지 서비스**

- **Role**: Full Stack (기획, 디자인, FE, BE)
- **Tech**: ![React](https://img.shields.io/badge/React-61DAFB?logo=react&logoColor=black) ![Spring Boot](https://img.shields.io/badge/SpringBoot-6DB33F?logo=springboot&logoColor=white) ![Oracle](https://img.shields.io/badge/Oracle-F80000?logo=oracle&logoColor=white) ![AWS S3](https://img.shields.io/badge/AWS%20S3-569A31?logo=amazonaws&logoColor=white) ![Figma](https://img.shields.io/badge/Figma-F24E1E?logo=figma&logoColor=white)
- **GitHub**: [Link](https://github.com/TikkleSaver)
- **Key Features**:
  - **Infra**: AWS S3 버킷 설정 및 이미지 업로드 처리
  - **Frontend**: 프로젝트 초기 세팅, 공통 헤더/푸터 컴포넌트 구현
  - **Logic**: 챌린지 생성 및 미션 인증 기능 전반(FE/BE) 개발

<br>

### 3. 어디약 (PharmQuest)
> **해외 여행객을 위한 현지 약국 및 상비약 정보 제공 서비스**
> <br/> *2024.01 - 2024.04*

- **Role**: Backend Developer
- **Tech**: ![Spring Boot](https://img.shields.io/badge/SpringBoot-6DB33F?logo=springboot&logoColor=white) ![MySQL](https://img.shields.io/badge/MySQL-4479A1?logo=mysql&logoColor=white)
- **GitHub**: [Link](https://github.com/PharmQuest/BE)
- **Key Features**:
  - **Community**: 게시글 작성/수정/삭제 및 인기글(Best) 선정 로직
  - **Interaction**: 좋아요, 스크랩, 신고 기능 및 대댓글(답글) 구조 구현

<br>

### 4. 에듀매니저 (EduManager)
> **학원생 강의 수강 및 스터디 관리 시스템**
> <br/> *2024.11 - 2024.12 (데이터베이스프로그래밍 최종 프로젝트)*

- **Role**: Full Stack (기획, 디자인, FE, BE)
- **Tech**: ![Java](https://img.shields.io/badge/Java-007396?logo=openjdk&logoColor=white) ![Oracle](https://img.shields.io/badge/Oracle-F80000?logo=oracle&logoColor=white) ![JSP](https://img.shields.io/badge/JSP-000000?logo=java&logoColor=white) ![HTML/CSS](https://img.shields.io/badge/HTML%2FCSS-E34F26?logo=html5&logoColor=white)
- **GitHub**: [Link](https://github.com/heessunny/EduManager-BBP)
- **Key Features**:
  - 강의 및 스터디 신청/목록 조회/찜하기 기능 구현
  - 수강 중인 강의/스터디 목록 및 공지사항, 과제 전체 보기
  - 후기 작성(Review) 기능 구현

<br>

### 5. 책모아 (BookMoA)
> **흩어진 독서 기록을 한눈에 모아보는 독서 관리 앱**

- **Role**: Android Developer (Frontend)
- **Tech**: ![Android](https://img.shields.io/badge/Android-3DDC84?logo=android&logoColor=white) ![Kotlin](https://img.shields.io/badge/Kotlin-7F52FF?logo=kotlin&logoColor=white)
- **GitHub**: [Link](https://github.com/BookMoA/Frontend)
- **Key Features**:
  - **Library**: 내 독서 리스트(읽은 책/읽고 있는 책) 관리 및 수정/삭제
  - **Search**: 알라딘 API 연동을 통한 도서(제목/저자) 검색 기능
  - **Recommendation**: 인기 도서 Top 10 및 추천 도서 UI 구현

<br>

### 6. 동네형 (Fitness Bro)
> **운동 메이트 매칭 및 소통을 위한 피트니스 웹 서비스**

- **Role**: Frontend Developer (Web)
- **Tech**: ![React](https://img.shields.io/badge/React-61DAFB?logo=react&logoColor=black)
- **GitHub**: [Link](https://github.com/fitness-bro/Frontend.git)
- **Key Features**:
  - 회원가입/정보수정 및 마이페이지 UI 구현
  - 실시간 채팅방 리스트 및 채팅 화면 인터페이스 개발

<br>

<div align="center">
  <a href="https://github.com/anuraghazra/github-readme-stats">
    <img src="https://github-readme-stats.vercel.app/api?username=heessunny&show_icons=true&theme=radical" alt="heessunny's github stats" />
  </a>
</div>
